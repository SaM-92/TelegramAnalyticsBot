...
def process_time_resolution_and_duplicates(
    df_read,
    time_column,
...
def display_column_statistics(df_read):
    ...
    """
    all_statistics_texts = []
    for column in df_read.columns:
        # Escape the column name for Telegram-compatible markdown formatting
        column_escaped = escape_markdown(column)

        # Compute descriptive statistics for the current column
        stats = df_read[column].describe()

        # Construct the summary text with Telegram-compatible markdown formatting
        summary_text = (
            f"*Statistics for {column_escaped}*\n"
            f"Count: `{stats['count']}` data points\n"
            f"Mean: The average is `{stats['mean']:.2f}`\n"
            f"Standard Deviation: The standard deviation is `{stats['std']:.2f}`\n"
            f"Minimum: The smallest observed value is `{stats['min']:.2f}`\n"
            f"25th Percentile: 25% of the values are `{stats['25%']:.2f}` or less\n"
            f"50th Percentile Median: The median value is `{stats['50%']:.2f}`\n"
            f"75th Percentile: 75% of the values are `{stats['75%']:.2f}` or less\n"
            f"Maximum: The largest observed value is `{stats['max']:.2f}`\n"
        )

        # Send the summary using Telegram's reply_text function
        all_statistics_texts.append(summary_text)

    return "\n".join(all_statistics_texts)
